<template>
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="宠物分类" prop="meizhuangfenlei">
					<el-input  v-model="ruleForm.meizhuangfenlei" placeholder="宠物分类" clearable  :readonly="ro.meizhuangfenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' v-else class="input" label="宠物分类" prop="meizhuangfenlei">
					<el-input v-model="ruleForm.meizhuangfenlei" placeholder="宠物分类" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `meizhuangfenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.meizhuangfenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
